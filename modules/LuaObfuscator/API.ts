...
export default {
    v1: {
        CreateSession: async function (script: string): Promise<ObfuscationResult> {
            try {
                const response = await fetch(`https://luaobfuscator.com/api/obfuscator/newscript`, { method: "POST", body: script, headers: { apiKey: process.env.LUAOBF_APIKEY } })
        ...
        Obfuscate: async function (script: string): Promise<ObfuscationResult> {
            const response = await fetch(`https://luaobfuscator.com/api/obfuscator/one-click/hard`, { method: "POST", body: script, headers: { apiKey: process.env.LUAOBF_APIKEY } }).catch(error => {
                throw error
            })
